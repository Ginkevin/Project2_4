...
session_start();
	//initialise
		...
		$random_connect->getFriends($_GET["name"]); 
...
function showResult(str) {
  ...
  xmlhttp.send();
	...
	<body id="top">
		<!-- Header -->
			<header id="header">
				 <img src="'. $person[0]["avatar_url"] .'" alt="" height="300" width="400" />
				 </br></br></br>
		...
		<!-- Main -->
			<div id="main">
				...
				<!-- One -->
					<section id="one">
						<header class="major">
							<h2>'. $person[0]["first_name"] . " " . $person[0]["last_name"] .'</h2>
						</header>
					...
					<section id="two">
						...
						<div class="row">
							...
							';	for ($i = 0; $i < sizeof($friendlist); $i++){
								if ($i < 5){
									if($friendlist[$i]["first_name"] != $person[0]["first_name"]){
